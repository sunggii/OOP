...
public:
    // Default constructor
    Gem() { setType(5); count++; cout << "count Obj = " << count << endl;}

    // Constructor กำหนดค่าเริ่มต้น
    Gem(int t) { setType(5); count++; cout << "count Obj = " << count << endl;}

    // Destructor
    ~Gem() { count--; cout << "Delete Gem: " << "Type "<< type <<endl;}

    // Setter
    void setType(int t) { if (t >= 1 && t <= 5) {type = t;} }
...
public:
    ...
    // Setter
    void setJewelry(double p, int m){if (p > 0) {price = p;} if (m >= 1 && m <= 3) {madeFrom = m;}}
    void setPrice(double p) { if (p > 0) price = p; }
    void setMadeFrom(int m) { if (m >= 1 && m <= 3) madeFrom = m; }
...
public:
    // Default constructor
    Ring():Jewelry(){ setJewelry(3000,3); setNum(0);}

    // Constructor กำหนดค่าเริ่มต้น
    Ring(int n, double p, int m ): Jewelry(p,m) { setNum(n); }
...
public:
    Necklace():Jewelry(){ setJewelry(22000,1); setPendant(true);}
    Necklace(double p, int m, bool pd) : Jewelry(p, m){setPendant(pd);}
    ~Necklace(){cout<<"\nDelete Necklace "<<" Made From:" <<getMadeFrom()
                    << " Price:" <<getPrice()<<" Pendant: " <<pendant<< endl;
    }
   ...
   // แสดงรายละเอียดของ g
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        switch (r[i].getMadeFrom()) {
            case 1: cout << "ทอง"; break;
            case 2: cout << "เงิน"; break;
            case 3: cout << "ทองคำขาว"; break;
        }
        cout << endl;
    }
    ...
    int goldCount = 0;
    if (r[0].getMadeFrom() == 1) goldCount++;
    if (r[1].getMadeFrom() == 1) goldCount++;
    if (n[0].getMadeFrom() == 1) goldCount++;
    if (n[1].getMadeFrom() == 1) goldCount++;
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        cout << "Made from: " << r[i].getMadeFrom() <<endl;
    }
